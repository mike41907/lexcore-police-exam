...
const titles={home:["警佐班一類法規總覽","只保留考試需要的法規，不混入行政執行法、集會遊行法"],dashboard:["備考戰情室","把錯題、倒扣、信心與新修法轉成今天最值得讀的清單"],quiz:["題庫訓練","正式倒扣計分＋選項級陷阱分析＋官方歷屆題匯入"],amendments:["全部修法","民國111～115年所有修法事件跨法規一次看"],tree:["警察法規體系","母法、直接授權子法與相關獨立法律分層閱讀"],study:["修法／法條","有修法看新舊差異，無修法直接進必讀條文"],reasons:["立法理由","先理解為什麼修，再記條文"],exam:["考前必讀修法","依重要度、陷阱與完成狀態集中複習"],court:["憲法法庭","重大憲判、爭點與刑法／刑訴／憲訴法關聯"],sync:["同步狀態","官方法規、立法院與憲法法庭資料同步"],scope:["考試範圍","依警佐班一類科目整理"]};
let activeCat="警察法規";
...
const ATTEMPT_KEY="lexcore-attempts-v11";
...
function renderQuizQuestion(){
 ...
 $("quizArea").innerHTML=`<div class="qcard">
   <div class="qmeta"><span class="badge ${q.sourceType==="official"?"green":"amber"}">${q.sourceType==="official"?`${q.year}官方歷屆`:"系統診斷題"}</span><span class="badge blue">${q.subject}</span><span class="badge ${q.type==="single"?"blue":"red"}">${q.type==="single"?"單選":"複選"}</span><span class="badge amber">${QUIZ.index+1}/${QUIZ.questions.length}</span></div>
...
function warRows(){
 ...
 for(const x of Object.values(map)){
   ...
   const heat=(meta.score||3)/5;
   x.risk=Math.round(100*(.28*err+.19*loss+.17*conf+.10*slow+.11*recency+.15*heat));
...
function renderWarRoom(){
 ...
 $("dailyList").innerHTML=rows.slice(0,10).map((x,i)=>`<div class="daily-item">
   <div class="row"><div><h3>${i+1}. ${x.label}</h3><div class="meta">風險 ${x.risk}　作答 ${x.n} 次　錯 ${x.wrong} 次${x.avgsec?`　平均 ${x.avgsec}s`:""}</div></div><span class="badge ${x.risk>=65?"red":x.risk>=45?"amber":"green"}">${x.risk>=65?"先救這個":x.risk>=45?"需補強":"維持"}</span></div>
...
function renderExamSources(){
 ...
 $("examSources").innerHTML=`<div class="memory">官方題庫狀態：${status}</div>`+EXAM_SOURCES.sources.map(s=>`<div class="source-row"><b>${s.rocYear}年</b><div>警佐班第${s.classNo}期　原始公告 ${s.published}<br><span class="meta">${(s.corrections||[]).map(c=>`更正版 ${c.published}`).join("；")||"無更正紀錄"}</span></div><a class="chip" style="text-decoration:none" href="${s.announcement}" target="_blank">官方來源</a></div>`).join("");
}
...
fillFilter();fillAmendLaw();fillTreeParent();renderTabs();renderAllAmendments();renderLawTree();renderStudy();renderReasons();renderExam();renderCourt();renderSync();renderExamSources();renderWarRoom();loadRuntime();loadQuestionBank();

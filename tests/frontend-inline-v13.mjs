...
function dedupePreview(rows){
 const seen=new Set(), existing=new Set(IMPORTED_QUESTIONS.map(x=>x.id));
 return rows.map(q=>({...q,isDuplicate:existing.has(q.id)||seen.has(q.id)})).filter(q=>{seen.add(q.id);return true});
}
...
function renderImportPreview(){
 ...
 $("importPreview").innerHTML=rows.slice(0,120).map((x,i)=>`<div class="import-item">
   <div class="row"><h4>${i+1}. ${esc(x.stem.slice(0,180))}${x.stem.length>180?"…":""}</h4><span class="badge ${x.isDuplicate?"amber":"green"}">${x.isDuplicate?"重複":"可匯入"}</span></div>
   ...
   <div class="import-answer">答案：${x.answer?.length?x.answer.join(""):"未辨識"}${x.myAnswer?.length?`　我的答案：${x.myAnswer.join("")}`:""}</div>
...
async function commitYamolImport(){
 ...
 $("importResult").innerHTML=`<div class="notice"><strong>已匯入 ${rows.length} 題。</strong> 已加入「我的阿摩錯題」題庫，並納入戰情室弱點排序。</div>`;
}
...
function warRows(){
 ...
 for(const q of IMPORTED_QUESTIONS){
   ...
   if(!map[key])map[key]={key,lawId:q.lawId,article:q.article,label:q.lawId?`${lawName(q.lawId)} 第${q.article}條`:key,n:0,wrong:0,loss:0,seconds:0,highConfWrong:0,last:q.importedAt||null,traps:{},yamolWrong:0};
 ...
 for(const x of Object.values(map)){
   ...
   x.risk=Math.round(100*(.24*err+.16*loss+.15*conf+.08*slow+.09*recency+.14*heat+.14*yamol));

...
const titles={home:["警佐班一類法規總覽","只保留考試需要的法規，不混入行政執行法、集會遊行法"],amendments:["全部修法","民國111～115年所有修法事件跨法規一次看"],tree:["警察法規體系","母法、直接授權子法與相關獨立法律分層閱讀"],study:["修法／法條","有修法看新舊差異，無修法直接進必讀條文"],reasons:["立法理由","先理解為什麼修，再記條文"],exam:["考前必讀修法","依重要度、陷阱與完成狀態集中複習"],court:["憲法法庭","重大憲判、爭點與刑法／刑訴／憲訴法關聯"],sync:["同步狀態","官方法規、立法院與憲法法庭資料同步"],scope:["考試範圍","依警佐班一類科目整理"]};
let activeCat="警察法規";
...
$("lawFilter").onchange=renderStudy;$("search").oninput=renderStudy;if($("amendSearch"))$("amendSearch").oninput=renderAllAmendments;if($("amendYear"))$("amendYear").onchange=renderAllAmendments;if($("amendLaw"))$("amendLaw").onchange=renderAllAmendments;if($("courtSearch"))$("courtSearch").oninput=renderCourt;if($("courtYear"))$("courtYear").onchange=renderCourt;if($("treeSearch"))$("treeSearch").oninput=renderLawTree;if($("treeParent"))$("treeParent").onchange=renderLawTree;if($("examSearch"))$("examSearch").oninput=renderExam;if($("examScore"))$("examScore").onchange=renderExam;if($("examStatus"))$("examStatus").onchange=renderExam;
...
fillFilter();fillAmendLaw();fillTreeParent();renderTabs();renderAllAmendments();renderLawTree();renderStudy();renderReasons();renderExam();renderCourt();renderSync();loadRuntime();
